...
- **API**: RESTful with Swagger documentation at `/docs`
...
- **calendar**: Google Calendar API wrapper (OAuth por actor: company/staff/client)
...
- **PostgreSQL**: Database (`DATABASE_URL` or individual DB\_\* vars)
- **Google Calendar OAuth**: (`GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `GOOGLE_CALENDAR_CALLBACK_URL`, `ENCRYPTION_KEY`, `WEBHOOK_BASE_URL`)
